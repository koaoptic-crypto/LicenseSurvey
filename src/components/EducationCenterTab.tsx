...
export function EducationCenterTab() {
  ...
  const processFiles = async () => {
    ...
    for (const file of files) {
      ...
      headers.forEach((header, index) => {
        ...
        } else if (normalizedHeader === '보수교육년도' || normalizedHeader === '대상연도') {
          columnIndices['대상연도'] = index
        } else if (normalizedHeader === '구분') {
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, '보수교육_면허신고센터_처리결과.xlsx')
  }
    ...
    <div className="space-y-6">
      <Card>
        <CardContent className="p-6">
          <h2 className="text-xl font-semibold mb-4">보수교육(면허신고센터)</h2>
