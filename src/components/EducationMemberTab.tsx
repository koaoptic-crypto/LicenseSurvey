...
export function EducationMemberTab() {
  ...
  const processFiles = async () => {
    ...
    for (const file of files) {
      ...
      const educationDateIndices: number[] = []
      ...
      headers.forEach((header, index) => {
        ...
        } else if (normalizedHeader?.includes('교육일시')) {
          educationDateIndices.push(index)
        }
      })
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        // 교육일시 컬럼들에서 연도 추출
        let targetYear = ''
        for (const dateIndex of educationDateIndices) {
          const year = extractYear(row[dateIndex]?.toString())
          if (year) {
            targetYear = year
            break
          }
    ...
    <div className="space-y-6">
      <Card>
        <CardContent className="p-6">
          ...
          <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-md p-4 mb-4 space-y-2">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  <li>다운로드 받은 엑셀 파일을 <strong>"다른 이름으로 저장"</strong>을 눌러 파일 형식을 <strong>"Excel 통합 문서(.xlsx)"</strong>로 저장한 후 업로드해주세요.</li>
